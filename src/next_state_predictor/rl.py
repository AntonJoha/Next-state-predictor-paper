...
def _get_optimizer_lambda(
    ...
    if hasattr(args, "optimizer") and args.optimizer == "adam":
        return lambda parameters: torch.optim.Adam(
            parameters, lr=args.lr if hasattr(args, "lr") else 0.001
        )
    elif hasattr(args, "optimizer") and args.optimizer == "sgd":
        return lambda parameters: torch.optim.SGD(
            parameters, lr=args.lr if hasattr(args, "lr") else 0.001
        )
    else:
...
def _evaluate_model(
    model: torch.nn.Module, env: gym.Env, num_episodes: int = 10
) -> float:
    total_reward = 0.0
...
def train_rl(args) -> None:
    ...
    output_file = os.path.join(
        args.rl_output_dir, f"length_{args.num_episodes}_id_{str(time.time())}.npz"
    )
    ...
    for episode in range(args.num_episodes):
        ...
        state_trajectory = np.zeros(
            (args.max_steps, env.observation_space.shape[0] + env.action_space.n)
        )  # +1 for reward
        ...
        while not done and step < args.max_steps:
            ...
            state_trajectory[
                step,
                env.observation_space.shape[0] : env.observation_space.shape[0]
                + env.action_space.n,
            ] = action
            states.append(state)
        ...
        print(
            f"Episode {episode + 1}/{args.num_episodes}, Score: {score}, Epsilon: {epsilon:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_episodes", type=int, default=500, help="Number of episodes to run"
    )
    parser.add_argument(
        "--max_steps", type=int, default=200, help="Maximum number of steps per episode"
    )
    parser.add_argument(
        "--rl_output_dir", type=str, default="rl_data", help="Directory to save RL data"
    )
    parser.add_argument(
